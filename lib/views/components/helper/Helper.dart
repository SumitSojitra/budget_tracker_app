...
class DbHelper extends GetxController {
  ...
  Future initDb() async {
    ...
    db = await openDatabase(
      ...
      onCreate: (Database db, int version) async {
        await db.execute(
            "CREATE TABLE IF NOT EXISTS budget(id INTEGER PRIMARY KEY AUTOINCREMENT,category_name TEXT NOT NULL,category_image BLOB NOT NULL)");
      },
    );
    print("=========================");
    print("dbjd");
  ...
  Future<List<BudgetModel>>? FetchAllCategory() async {
    ...
    List<BudgetModel> alldata =
        res!.map((e) => BudgetModel.sql(data: e)).toList();
    return alldata;
  }
  ...
  Future<int?> DeleteData({required int id}) async {
    ...
    List arg = [id];
    int? res = await db?.rawDelete(query, arg);
    return res;
  }
}
